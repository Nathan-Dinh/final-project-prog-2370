...
namespace NDJPFinal.Source.Sprites
{
    public class ScrolllingBackground : Sprite
    {
        ...
        public ScrolllingBackground(Texture2D texture,float layer,Vector2 position) : base(texture,layer)
        {
            ...
            this._position2 = new Vector2(_position1.X, (_position1.Y - _currentAnimationFrames[0].Height));
        }
        ...
        {
            spriteBatch.Draw(_backgroundTexture, _position1, _currentAnimationFrames[_currentImage2], Color.White, 0f, new Vector2(0, 0), 1f, SpriteEffects.None, 0.0f);
            spriteBatch.Draw(_backgroundTexture, _position2, _currentAnimationFrames[_currentImage1], Color.White,0f,new Vector2(0,0), 1f, SpriteEffects.None, 0.0f);
        }
        ...
        private void MoveBackground(GameTime gameTime)
        {
            ...
            this._position1 += _velocity * (float)gameTime.ElapsedGameTime.TotalMilliseconds * 0.05f;
            this._position2 += _velocity * (float)gameTime.ElapsedGameTime.TotalMilliseconds * 0.05f;

            // Check if the first background section has moved beyond its height
            if (_position1.Y > _currentAnimationFrames[0].Height)
            {
                ...
                _position1.Y = _position2.Y - _currentAnimationFrames[_currentImage2].Height;
            }

            // Check if the second background section has moved beyond its height
            if (_position2.Y > _currentAnimationFrames[0].Height)
            {
                ...
                _position2.Y = _position1.X - _currentAnimationFrames[_currentImage1].Height;
            }
        }
    }
}
